...
日付計算・カードの締め日と引き落とし日・残高推移の生成をテストしています。数字の出方を変えるときは、まずここが落ちるかを見てください。
...
- カード利用の計上は、今日ではなく「カード情報の時点日」から数えます。時点日から今日までの利用は、利用可能額にも未確定利用額にもまだ入っていないためです
- すでに過ぎた引き落としは、今日の口座残高に反映済みとみなして計上しません
...
- `src/components/BalanceTrendChart.tsx`: 口座別の残高推移チャート（依存ライブラリなしの手書き SVG）
